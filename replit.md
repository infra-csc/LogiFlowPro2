...
- **Loading Orders** (Oct 21, 2025): Complete loading order system for consolidating approved material requests into picking lists. Backend implements three tables: `loading_orders` (main order with planned times, status, creator), `loading_order_requests` (many-to-many junction linking orders to requests), and `loading_order_items` (consolidated items with source tracking in JSONB). Consolidation logic in `loadingOrderUtils.ts` expands parametric kits using BOM formulas (evaluated with kit parameters), groups identical products across requests, and tracks source breakdown (requestId, area, quantity per source). Six API endpoints: GET list, GET detail, GET items, GET requests, POST create (with automatic consolidation), PATCH update, POST approve, POST disapprove. Frontend provides three components: `/loading-orders` list page with status badges and planned times, `/loading-orders/:id` detail page showing consolidated items with source breakdown badges and approval controls, and `LoadingOrderDialog` for creating new orders by selecting event and approved requests. Status enum: draft, ready, approved, in_progress, completed, cancelled. Approval flow: orders can be approved from ready/draft status, disapproved orders return to draft. Query keys use full URLs (`/api/loading-orders/${id}`) with `enabled: !!id` to prevent premature fetching.
- **Warehouse Movements (Carga e Descarga)** (Oct 21, 2025): Complete warehouse operations module for managing loading/unloading operations with scanner interface. Backend implements two tables: `movements` (main movement with type, status, loading order reference, vehicle, dock, timestamps), `movement_items` (scanned items with quantities). Movement types: outbound_event, inbound_event, inbound_purchase, inbound_rental, outbound_rental_return, internal_transfer, inventory_adjustment. Status transitions validated server-side: created → in_progress → paused/completed. Items can only be added when movement is in_progress. Six API endpoints with validation: GET list, GET detail, GET items, POST create (validates loading order is approved), PATCH update (validates status transitions, sets timestamps), POST items (enforces movementId integrity). Frontend provides three pages: `/movements` list with status badges and inline action buttons (start/pause/finish/continue), `/movements/:id` operational interface with product scanner (search by SKU/barcode/name), quantity controls, real-time progress tracking, and `MovementDialog` for creating new movements. All mutations implement proper error handling with res.ok checks and error toasts. Optional fields (loadingOrderId, vehiclePlate) normalized to undefined when empty to prevent validation errors.
