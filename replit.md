...
- **3.2.1** (2026-05-29): Refinamento Visual de Movimentações — componentes do Design System aplicados em `movements.tsx` (PageHeader, PageLoading, EmptyState, StatusBadge, FilterBar), `movement-details.tsx` (PageHeader, PageLoading, StatusBadge), `movement-approvals.tsx` (PageHeader, PageLoading, EmptyState). Removidos `getStatusColor`/`getStatusLabel` locais. `npm run check` e `build` zerados. Back-end, RBAC, API, regras de negócio, scanner, audit logs intactos. Detalhes em [`docs/CHANGELOG-fase3.md`](docs/CHANGELOG-fase3.md).
