    ...
    <div className="border border-border/60 rounded-md p-3 hover-elevate space-y-2">
      ...
      {showMov && product.movements && product.movements.length > 0 && (
        <div className="pt-2 border-t border-border/40 space-y-1.5">
          ...
          {product.movements.slice(0, 10).map((m: any, idx: number) => (
            <div key={m.id ?? idx} className="rounded-md border border-border/40 bg-muted/30 px-3 py-2 space-y-1.5">
              {/* Row 1: direction badge + date + qty */}
              <div className="flex items-center gap-2 flex-wrap">
                <Badge
                  variant={m.direction === "in" ? "default" : "destructive"}
                  className="text-[10px] no-default-hover-elevate flex-shrink-0"
                >
                  {m.direction === "in" ? "Entrada" : "Saída"}
                </Badge>
                {m.typeName && (
                  <span className="text-xs font-medium">{m.typeName}</span>
                )}
                <span className="text-xs text-muted-foreground ml-auto">
                  {m.date ? format(new Date(m.date), "dd/MM/yy HH:mm", { locale: ptBR }) : "—"}
                </span>
                <span className={cn(
                  "text-sm font-bold flex-shrink-0",
                  m.direction === "in" ? "text-chart-4" : "text-destructive"
                )}>
                  {m.direction === "in" ? "+" : "−"}{m.quantity} un
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 flex-wrap text-xs text-muted-foreground">
                ...
                {m.ownerName && (
                  <span className="text-muted-foreground">{m.ownerType ? `${m.ownerType}: ` : ""}{m.ownerName}</span>
                )}
                {m.status && (
                  <StatusBadge status={m.status} className="text-[10px] no-default-hover-elevate ml-auto" />
                )}
              </div>
