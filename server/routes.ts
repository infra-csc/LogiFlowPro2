...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/cleanup-prod-data", requireAdmin({ message: "Admin only" }), async (req, res) => {
    try {
      const { pool } = await import("./db");
      const skus = ['5000','15','PR01','GRA','PRAT','P01','CAD-001','CAD-LOC-001','CAD-TERC-001','MESA-001','BOX_3QC5_LOC','BOX_3QC3','BOX_3QC4','BOX_3QC5','TDA_CTNR'];
      const skuList = skus.map((_,i) => `$${i+1}`).join(',');

      const steps: Record<string, number> = {};

      // 1. movement_audit_logs
      const r1 = await pool.query(`DELETE FROM movement_audit_logs`);
      steps.movement_audit_logs = r1.rowCount ?? 0;

      // 2. movement_attachments
      const r2 = await pool.query(`DELETE FROM movement_attachments`);
      steps.movement_attachments = r2.rowCount ?? 0;

      // 3. movement_items
      const r3 = await pool.query(`DELETE FROM movement_items`);
      steps.movement_items = r3.rowCount ?? 0;

      // 4. movements
      const r4 = await pool.query(`DELETE FROM movements`);
      steps.movements = r4.rowCount ?? 0;

      // 5. loading_order_items
      const r5 = await pool.query(`DELETE FROM loading_order_items`);
      steps.loading_order_items = r5.rowCount ?? 0;

      // 6. loading_orders
      const r6 = await pool.query(`DELETE FROM loading_orders`);
      steps.loading_orders = r6.rowCount ?? 0;

      // 7. returns
      const r7 = await pool.query(`DELETE FROM returns`);
      steps.returns = r7.rowCount ?? 0;

      // 8. trip_items
      const r8 = await pool.query(`DELETE FROM trip_items`);
      steps.trip_items = r8.rowCount ?? 0;

      // 9. trips
      const r9 = await pool.query(`DELETE FROM trips`);
      steps.trips = r9.rowCount ?? 0;

      // 10. cascade children of products-to-delete
      await pool.query(`
        DELETE FROM movement_attachments
        WHERE movement_item_id IN (
          SELECT id FROM movement_items WHERE product_id IN (
            SELECT id FROM products WHERE sku = ANY($1::text[])
          )
        ) OR product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))
      `, [skus]);
      await pool.query(`DELETE FROM movement_items WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM request_items WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM loading_order_items WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM bom_lines WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM returns WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM trip_items WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM batch_lots WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM inventory_movements WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM inventory_snapshots WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);
      await pool.query(`DELETE FROM request_area_template_items WHERE product_id IN (SELECT id FROM products WHERE sku = ANY($1::text[]))`, [skus]);

      // 11. delete extra products
      const rp = await pool.query(`DELETE FROM products WHERE sku = ANY($1::text[]) RETURNING sku`, [skus]);
      steps.products_deleted = rp.rowCount ?? 0;

      // 12. fix wrong name
      await pool.query(`UPDATE products SET name = 'Box Truss - Q30 - Peça 5000' WHERE sku = 'BOX_3Q50'`);

      // final count
      const count = await pool.query(`SELECT COUNT(*) as total FROM products`);
      steps.products_remaining = Number(count.rows[0].total);
